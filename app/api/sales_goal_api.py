...
sales_goal_api = Blueprint('sales_goal_api', __name__, url_prefix='/api/sales-goals')
...
def get_all():
    """Lista todos los meta de ventas con paginación"""
    try:
...
def get_by_id(id):
    """Obtiene un meta de venta por ID"""
    try:
        obj = handler.get(id)
        if obj:
            return success_response(obj.to_dict())
        return error_response('Meta de venta no encontrado', 404)
    except Exception as e:
        return error_response(str(e), 500)
...
def create():
    """Crea un nuevo meta de venta"""
    try:
        data = request.get_json()
        # Invalidar cache
        cache.delete_memoized(get_all)
        obj = handler.create(**data)
        return success_response(obj.to_dict(), 'Meta de venta creado exitosamente', 201)
...
def update(id):
    ...
    try:
        ...
        return success_response(obj.to_dict(), 'Meta de venta actualizado exitosamente')
    except ValueError as e:
...
def delete(id):
    ...
    try:
        ...
        if deleted:
            return success_response(message='Meta de venta eliminado exitosamente')
        return error_response('Meta de venta no encontrado', 404)
    except Exception as e:
        return error_response(str(e), 500)
