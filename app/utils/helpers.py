...
def paginate_query(query, page=1, per_page=10):
    ...
    Example:
        query = Quote.query.filter_by(status='active')
...
def success_response(data=None, message="Operación exitosa", status_code=200):
    ...
    Example:
        # Con datos
        return success_response(quote.to_dict(), "Quote created")
...
def error_response(error_message, status_code=400, error_code=None):
    ...
    Example:
        ...
        # Con código de error
        return error_response("Invalid data", 400, "VALIDATION_ERROR")
    """
    response = {
        'success': False,
        'error': error_message
...
def parse_status_filter():
    ...
    Example:
        status = parse_status_filter()
        result = handler.list_all(page=1, status=status)
    """
...
def parse_filters(*allowed_filters):
    ...
    Example:
        # En endpoint
        filters = parse_filters('organization_id', 'status', 'client_name')
        # Si request tiene ?organization_id=1&status=active
        # Retorna {'organization_id': '1', 'status': 'active'}
    """
    filters = {}
...
def safe_int(value, default=None):
    ...
    Example:
        organization_id = safe_int(request.args.get('organization_id'))
        if organization_id:
